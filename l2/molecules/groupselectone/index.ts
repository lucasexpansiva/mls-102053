...
export class GroupSelectOneIndex extends StateLitElement {
    ...
    return html`
      <header class="px-8 py-20 text-center" style="background: rgba(255,255,255,0.06); border-bottom: 1px solid rgba(255,255,255,0.25); backdrop-filter: blur(10px);">
        ...
        <p class="text-lg max-w-2xl mx-auto leading-relaxed" style="color: rgba(255,255,255,0.72);">
          ...
          Ideal for scenarios where a single, clear decision is required. Layout is chosen via the variant property.
        </p>
      </header>
    `;
  }
          ...
          <div class="rounded-2xl" style="background: rgba(255,255,255,0.10); border: 1px solid rgba(255,255,255,0.25); box-shadow: 0 4px 18px rgba(0,0,0,0.18); backdrop-filter: blur(10px);">
            ...
            <div class="p-6">
              <div class="flex items-center justify-between mb-1">
                ...
                <code class="text-xs px-2 py-0.5 rounded" style="background: rgba(255,255,255,0.12); color: rgba(255,255,255,0.65);">groupselectone--ml-discrete-slider-glass</code>
              </div>
              <p class="text-xs mb-5" style="color: rgba(255,255,255,0.55);">Glass discrete slider for picking exactly one option along a stepped track</p>
              <groupselectone--ml-discrete-slider-glass
                ...
                <Helper>Slide to choose a single plan tier</Helper>
  ...
  private renderReferenceTable(): TemplateResult {
    const rows: Array<{ scenario: string; discreteSlider: boolean }> = [
      { scenario: 'Pick one value from a small ordered set of steps', discreteSlider: true },
      { scenario: 'Visually communicate rank or intensity along a track', discreteSlider: true },
      { scenario: 'Compare multi-attribute options side-by-side in columns', discreteSlider: false },
      { scenario: 'Compact single-choice control with glassmorphism styling', discreteSlider: true },
      { scenario: 'Searchable long list of mutually exclusive labels', discreteSlider: false },
    ];
    const headers = [
    ...
    return html`
      <section class="px-8 py-20" style="background: rgba(255,255,255,0.06); border-top: 1px solid rgba(255,255,255,0.25);">
        <div class="max-w-5xl mx-auto">
          ...
          <p class="text-sm mb-8" style="color: rgba(255,255,255,0.65);">When to use the discrete slider variant for mutually exclusive single selection</p>
              ...
              <tbody>
                ${rows.map((row, i) => html`
                  <tr class="last:border-0" style="${i % 2 !== 0 ? 'background: rgba(255,255,255,0.04);' : ''} border-bottom: 1px solid rgba(255,255,255,0.12);">
                    ...
                    ${([row.discreteSlider] as boolean[]).map(ok => html`
                      <td class="px-4 py-3.5 text-center">
                        ${ok
                          ? html`<span class="inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold" style="background: rgba(16,185,129,0.25); color: rgb(110,231,183);">✓</span>`

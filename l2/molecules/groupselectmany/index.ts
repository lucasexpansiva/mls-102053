...
export class GroupSelectManyIndex extends StateLitElement {
  ...
  private renderHero(): TemplateResult {
    return html`
      <header class="px-8 py-20 text-center" style="background: rgba(255,255,255,0.06); border-bottom: 1px solid rgba(255,255,255,0.25); backdrop-filter: blur(10px);">
        <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-widest mb-6"
        ...
        <p class="text-lg max-w-2xl mx-auto leading-relaxed" style="color: rgba(255,255,255,0.65);">
          Allows the user to select one or more options from a list. Value is a comma-separated string of selected item values.
          Supports searchable filtering, min/max selection limits, grouped items, and disabled options.
        </p>
      </header>
    `;
  }
          ...
          <div class="rounded-2xl overflow-hidden" style="background: rgba(255,255,255,0.10); border: 1px solid rgba(255,255,255,0.25); box-shadow: 0 4px 18px rgba(0,0,0,0.18); backdrop-filter: blur(10px);">
            ...
            <div class="p-6">
              <div class="flex items-center justify-between mb-1">
                <p class="text-sm font-bold" style="color: rgba(255,255,255,0.96);">Multi Checkbox List</p>
                <code class="text-xs px-2 py-0.5 rounded" style="background: rgba(255,255,255,0.12); color: rgba(255,255,255,0.55);">groupselectmany--ml-multi-checkbox-list-glass</code>
              </div>
              <p class="text-xs mb-5" style="color: rgba(255,255,255,0.45);">Glass checkbox group for multi-select permissions and option lists</p>
  ...
  private renderReferenceTable(): TemplateResult {
    const rows: Array<{ scenario: string; multiCheckboxList: boolean }> = [
      { scenario: 'Inline multi-select with visible checkboxes for a short option list', multiCheckboxList: true },
      { scenario: 'Permissions or feature flags where every option should stay on-screen', multiCheckboxList: true },
      { scenario: 'Grouped options under labeled headings (e.g. scopes by category)', multiCheckboxList: true },
      { scenario: 'Need min/max selection limits with clear multi-choice affordance', multiCheckboxList: true },
      { scenario: 'Dense comparison of multi-attribute rows (prefer a table variant)', multiCheckboxList: false },
      { scenario: 'Compact trigger that opens a panel (prefer a dropdown variant)', multiCheckboxList: false },
    ];
    const headers = [
    ...
    return html`
      <section class="px-8 py-20" style="background: rgba(255,255,255,0.04); border-top: 1px solid rgba(255,255,255,0.25);">
        <div class="max-w-5xl mx-auto">
          ...
          <p class="text-sm mb-8" style="color: rgba(255,255,255,0.55);">
            When to use the glass multi-checkbox list versus other select-many patterns
          </p>
          <div class="rounded-2xl overflow-hidden" style="background: rgba(255,255,255,0.10); border: 1px solid rgba(255,255,255,0.25); box-shadow: 0 4px 18px rgba(0,0,0,0.18); backdrop-filter: blur(10px);">
              ...
              <tbody>
                ${rows.map((row, i) => html`
                  <tr class="last:border-0" style="${i % 2 !== 0 ? 'background: rgba(255,255,255,0.04);' : ''} border-bottom: 1px solid rgba(255,255,255,0.12);">
                    ...
                    ${([row.multiCheckboxList] as boolean[]).map(ok => html`
                      <td class="px-4 py-3.5 text-center">
                        ${ok
                          ? html`<span class="inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold" style="background: rgba(16,185,129,0.25); color: #6ee7b7;">✓</span>`
                          : html`<span class="text-sm" style="color: rgba(255,255,255,0.2);">—</span>`}
                      </td>
                    `)}
                  </tr>
                `)}

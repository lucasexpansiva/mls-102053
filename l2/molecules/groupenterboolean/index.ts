...
import '/_102053_/l2/molecules/groupenterboolean/ml-toggle-switch-glass';
...
export class GroupEnterBooleanIndex extends StateLitElement {
      ...
      <section class="px-8 py-12" style="border-bottom: 1px solid rgba(255,255,255,0.25);">
        <div class="max-w-2xl mx-auto flex flex-col gap-5">
          ...
          <div class="rounded-2xl" style="background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.25); box-shadow: 0 4px 18px rgba(0,0,0,0.18); backdrop-filter: blur(10px);">
            ...
            <div class="p-6">
              <div class="flex items-center justify-between mb-1">
                <p class="text-sm font-bold" style="color: rgba(255,255,255,0.96);">Boolean Segmented</p>
                <code class="text-xs px-2 py-0.5 rounded" style="background: rgba(255,255,255,0.1); color: rgba(255,255,255,0.65);">groupenterboolean--ml-boolean-segmented-glass</code>
              </div>
              <p class="text-xs mb-5" style="color: rgba(255,255,255,0.55);">Segmented control for an explicit true/false choice</p>
              <groupenterboolean--ml-boolean-segmented-glass
                ...
                <Helper>Choose yes or no to toggle this setting</Helper>
              </groupenterboolean--ml-boolean-segmented-glass>
            </div>
          </div>
          ...
          <div class="rounded-2xl" style="background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.25); box-shadow: 0 4px 18px rgba(0,0,0,0.18); backdrop-filter: blur(10px);">
            ...
            <div class="p-6">
              <div class="flex items-center justify-between mb-1">
                <p class="text-sm font-bold" style="color: rgba(255,255,255,0.96);">Toggle Switch</p>
                <code class="text-xs px-2 py-0.5 rounded" style="background: rgba(255,255,255,0.1); color: rgba(255,255,255,0.65);">groupenterboolean--ml-toggle-switch-glass</code>
              </div>
              <p class="text-xs mb-5" style="color: rgba(255,255,255,0.55);">Classic on/off switch for binary settings</p>
              <groupenterboolean--ml-toggle-switch-glass
                ...
                <Helper>Receive alerts when something important happens</Helper>
              </groupenterboolean--ml-toggle-switch-glass>
            </div>
          </div>
  ...
  private renderReferenceTable(): TemplateResult {
    const rows: Array<{ scenario: string; segmented: boolean; toggle: boolean }> = [
      { scenario: 'Explicit yes/no choice with equal visual weight on both options', segmented: true, toggle: false },
      { scenario: 'Compact on/off setting in a dense settings panel', segmented: false, toggle: true },
      { scenario: 'Binary preference that should feel like a physical switch', segmented: false, toggle: true },
      { scenario: 'Form field where both true and false need equal prominence', segmented: true, toggle: false },
      { scenario: 'Interchangeable true/false input — same contract, swap the tag', segmented: true, toggle: true },
    ];
    const headers = [
      { label: 'Segmented', cls: 'text-violet-300' },
      { label: 'Toggle', cls: 'text-emerald-300' },
    ];
    ...
    return html`
      <section class="px-8 py-20" style="border-top: 1px solid rgba(255,255,255,0.25);">
        <div class="max-w-5xl mx-auto">
          ...
          <p class="text-sm mb-8" style="color: rgba(255,255,255,0.65);">Pick segmented when both options need equal weight; pick toggle for compact on/off settings — both share the same boolean contract.</p>
              ...
              <tbody>
                ${rows.map((row, i) => html`
                  <tr class="last:border-0" style="${i % 2 !== 0 ? 'background: rgba(255,255,255,0.04);' : ''} border-bottom: 1px solid rgba(255,255,255,0.12);">
                    ...
                    ${([row.segmented, row.toggle] as boolean[]).map(ok => html`
                      <td class="px-4 py-3.5 text-center">
                        ${ok
                          ? html`<span class="inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold" style="background: rgba(16,185,129,0.2); color: #34d399;">✓</span>`

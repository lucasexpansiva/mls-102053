...
export class GroupEnterBooleanIndex extends StateLitElement {
          ...
          >
            ...
            <div class="p-6">
              <div class="flex items-center justify-between mb-1">
                ...
                <code class="text-xs px-2 py-0.5 rounded" style="background: rgba(255,255,255,0.1); color: rgba(255,255,255,0.55);">groupenterboolean--ml-boolean-segmented-glass</code>
              </div>
              <p class="text-xs mb-5" style="color: rgba(255,255,255,0.45);">Segmented control for a true/false decision with glass styling</p>
  ...
  private renderReferenceTable(): TemplateResult {
    const rows: Array<{ scenario: string; segmented: boolean }> = [
      { scenario: 'Compact true/false choice with a segmented control look', segmented: true },
      { scenario: 'On/off setting that should read as a binary switch', segmented: true },
      { scenario: 'Terms acceptance or explicit yes/no confirmation', segmented: true },
      { scenario: 'Glass UI surfaces where the control must sit on a rich backdrop', segmented: true },
    ];
    const headers = [
      { label: 'Segmented Glass', cls: 'text-violet-300' },
    ];
      ...
      >
        <div class="max-w-5xl mx-auto">
          ...
          <p class="text-sm mb-8" style="color: rgba(255,255,255,0.55);">
            Pick the boolean control that matches your layout density and visual language — all share the same value contract and slots.
          </p>
          <div
            ...
            <table class="w-full text-sm">
              <thead>
                <tr style="background: rgba(255,255,255,0.06); border-bottom: 1px solid rgba(255,255,255,0.25);">
                  <th class="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wide w-3/4" style="color: rgba(255,255,255,0.5);">Scenario</th>
                  ${headers.map(h => html`
              ...
              <tbody>
                ${rows.map((row, i) => html`
                  ...
                  >
                    <td class="px-5 py-3.5" style="color: rgba(255,255,255,0.75);">${row.scenario}</td>
